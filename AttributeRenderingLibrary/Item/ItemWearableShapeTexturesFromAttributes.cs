...
public class ItemWearableShapeTexturesFromAttributes : ItemWearable, IShapeTexturesFromAttributes, IContainedMeshSource, IContainedCustomName, IAttachableToEntity, IWearableShapeSupplier
{
    ...
    protected new MeshData genMesh(ICoreClientAPI capi, ItemStack itemstack, ITexPositionSource texSource)
    {
        ...
        UniversalShapeTextureSource stexSource = new UniversalShapeTextureSource(clientApi, capi.ItemTextureAtlas, shape, rcshape.Base.ToString());
        ...
        newShape.StepParentShape(shape, "", shapePathForLogging.ToShortString(), capi.Logger, (key, code) => { });

        nowTesselatingShape = newShape;
        capi.Tesselator.TesselateShapeWithJointIds("entity", newShape, out mesh, stexSource, new Vec3f(), quantityElements: rcshape.QuantityElements, selectiveElements: rcshape.SelectiveElements);
    ...
    #region IAttachableToEntity
    void IAttachableToEntity.CollectTextures(ItemStack stack, Shape intoShape, string texturePrefixCode, Dictionary<string, CompositeTexture> intoDict)
    {
        foreach ((string textureCode, CompositeTexture texture) in stack.Item.Textures)
        {
            intoShape.Textures[textureCode] = texture.Baked.BakedName;
        }
        ...
        if (variants.FindByVariant(texturesByType, out Dictionary<string, CompositeTexture> _textures))
        {
            foreach ((string textureCode, CompositeTexture texture) in _textures)
            {
                ...
                intoShape.Textures[textureCode] = ctex.Baked.BakedName;
            }
        }
    }
    ...
    bool IAttachableToEntity.IsAttachable(Entity toEntity, ItemStack itemStack) => iattr != null;
