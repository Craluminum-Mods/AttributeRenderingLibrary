...
public class CollectibleBehaviorWearable(CollectibleObject collObj) : AttributeRenderingLibrary.CollectibleBehaviorWearableAttachment(collObj), IWearable, IWearableStatsSupplier
{
    ...
    public override string GetMeshCacheKey(ItemSlot slot, string slotType)
    {
        return slotType + "-wearableModelRef-" + $"{slot.Itemstack.Collectible.Code}-{Variants.FromStack(slot.Itemstack)}";
    }
    ...
    public override void LoadTypes(JsonObject properties)
    {
        ...
        Dictionary<string, string> unresolvedDressTypeByType = properties["clothesCategory"].AsObject<Dictionary<string, string>>();

        if (unresolvedDressTypeByType != null)
        {
            DressTypeByType ??= [];

            foreach ((string type, string dressType) in unresolvedDressTypeByType)
            {
                Enum.TryParse(dressType, true, out EnumCharacterDressType resolvedDressType);
                if (!DressTypeByType.ContainsKey(type))
                {
                    DressTypeByType.Add(type, resolvedDressType);
                }
            }
        }
    ...
    protected virtual void ensureConditionExists(ItemSlot slot, bool markdirty = true)
    {
        ...
        if (!slot.Itemstack.Attributes.HasAttribute("condition") && coreApi.Side == EnumAppSide.Server)
        {
            if (GetMaxWarmth(slot) != 0)
            {
                ...
                {
                    slot.Itemstack.Attributes.SetFloat("condition", (float)coreApi.World.Rand.NextDouble() * 0.4f);
                }
    ...
    {
        if (!slot.Itemstack.FindByVariant(ProtectionModifiersByType, out ProtectionModifiers protectionModifiers))
        {
            return null;
        }
